...
export default function Project({project}: Props) {
    return (
        <div className={'w-full relative bg-card-background rounded-3xl group flex flex-col'}>
            <div className={"relative h-80 w-full overflow-hidden"}>
                ...
                <div className={"absolute bottom-0 left-0 m-3 p-2 bg-white rounded-2xl flex flex-row gap-3 translate-y-16 group-hover:translate-y-0 transition-all"}>
            ...
            <div className={"p-4 flex flex-col gap-4"}>
                ...
                {project.link.length !== 0 ? <a className={"self-end"} target={'_blank'} href={project.link}><div className={""}>Voir sur github</div></a> : null}
            </div>
        </div>
    )
}
