...
const data: ProjectProps[] = [
  ...
  },
  {
    ...
    variant: 'yellow',
    icons: ['nestjs.png', 'react.png', 'grafana.svg', 'docker.png']
  },
  {
    title: "FBX-CLI",
  ...
  },
  {
    ...
    link: '',
    icons: ['nestjs.png', 'react.png', 'docker.png', 'k8s.svg', 'argocd.png']
  }
]
  ...
  return (
    <div className={"flex flex-col m-auto relative py-20 gap-12 lg:gap-24 project"}>
      <Container className={"flex flex-col gap-6 lg:flex-row justify-between lg:items-center relative m-auto"}>
        ...
        <p className={"text-[#E06141] cursor-pointer hover:underline"} onClick={() => window.open('https://github.com/frnikho', '_blank')}>Explorer tout mes projets</p>
      </Container>
      <div ref={ref} className={"flex flex-row gap-4 lg:gap-8 overflow-x-auto no-scrollbar pr-[4%] lg:pr-[8.17%] 3xl:pr-[16.5%]"} {...events}>
...
function Project({first = false, variant, title, description, image, icons}: ProjectProps & {first?: boolean}) {
  ...
  return (
    <div className={`select-none relative ${first ? 'ml-[4%] lg:ml-[8.17%] 3xl:ml-[16.5%]' : ''} min-w-[320px] lg:min-w-[600px] lg:min-h-[350px] ${color()} rounded-3xl`} onClick={onClickProjet}>
      <div className={"flex flex-col justify-between h-full"}>
        ...
        <div className={"flex flex-row self-center pb-4 lg:pb-8 px-4 lg:px-8"}>
          <img className={'select-none rounded-xl shadow-2xl h-[250px] lg:h-[350px] lg:w-[600px] object-cover object-top-left'} src={image} alt={title}/>
          <div className={'absolute bottom-0 right-0 p-4'}>
            <div className={'flex flex-row gap-2 bg-gray-50 drop-shadow-2xl px-3 py-1.5 rounded-full'}>
              {icons.map((icon) => <img key={icon} className={'h-7'} src={icon} alt={''}/>)}
            </div>
          </div>
        </div>
      </div>
