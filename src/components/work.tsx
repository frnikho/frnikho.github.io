...
export default function Work() {
    return (
        <div className={"bg-[#F8F7F1] flex flex-col py-28 gap-20 work"}>
...
function WorkItem({work}: { work: Work }) {
    ...
    return (
        <div className={"w-full project-card"}>
            <div className={"flex flex-row justify-between align-middle gap-12 transition-all"}>
                <div className={"flex-5/12 flex flex-col gap-0"}>
                    <Heading>{work.company}</Heading>
                    <Text className={"text-xs lg:text-md uppercase"}>{work.date}</Text>
                </div>
                <div className={"z-10"}>
                    <div className={`bg-[#F8F7F1] h-12 w-12 border-dashed rounded-full border-[#CCC] border-[2px] relative
                    before:absolute before:top-40px before:border-l-[2px] before:border-dashed before:border-l-[#CCC] before:content-[''] before:w-[2px] before:h-[200px] before:-translate-x-1/2 before:left-1/2 before:-z-10
                    `}>
                ...
                <div className={"flex-5/12 flex flex-col gap-4 h-40"}>
                    <Heading>{work.title}</Heading>
                    <Text className={"text-xs lg:text-md"}>{work.description}</Text>
                </div>
            </div>
        </div>
    )
